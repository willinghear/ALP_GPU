...
__device__ void rowgroup_sample_and_find_k_combinations(
    ...
    // 收集所有出现过的(e,f)组合
    EFCombination all_combinations[361];  // 最多19x19
...
template<typename T>
__global__ void kernel_compress_emit_parallel_with_sampling(
    const T* data,
    const uint64_t* blk_starts,
    const uint64_t* blk_sizes,
    const uint64_t* bit_offsets,
    ...
    // 第一级采样：决定模式和k个组合（线程0执行）
    __shared__ EFCombination sh_k_combinations[5];  // MAX_K_COMBINATIONS
    ...
    if(threadIdx.x == 0) {
        rowgroup_sample_and_find_k_combinations<T>(
            blk, n, vectorSize,
            sh_k_combinations, sh_k_actual, sh_mode
        );
    }
    __syncthreads();
    
    // 如果是ALPrd模式，使用原有逻辑
    if(sh_mode == CompressionMode::ALPrd) {
        // 限制：如果向量太多，回退到串行
        if (numVec > MAX_VECS_PER_BLOCK) {
            if (threadIdx.x == 0) {
                BitWriter bw{out_bytes, bit_offsets[blockId]};
    ...
    if (numVec > MAX_VECS_PER_BLOCK) {
        if (threadIdx.x == 0) {
            ...
            for(int v = 0; v < numVec; v++) {
                int vec_start = v * vectorSize;
                int vec_size = min(vectorSize, n - vec_start);
                ...
                // 写入
                alp_vector_write<T>(bw, blk + vec_start, vec_size, e, f, bitw, FOR);
            }
        }
        return;
    }
